...
@section('title', 'Nueva ambulancia')
@section('header', 'NUEVA AMBULANCIA')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-md overflow-hidden">
        ...
        <div class="bg-[#d90000] px-8 py-5">
            <h3 class="text-white text-xl font-semibold">Registrar ambulancia</h3>
            <p class="text-red-200 text-sm mt-1">Completa los campos para registrar una nueva ambulancia.</p>
        </div>

        {{-- Formulario --}}
        <form action="{{ route('ambulancias.store') }}" method="POST" class="px-8 py-8 space-y-6">
            @csrf
                    ...
                    <select name="id_tipo_ambulancia"
                        ...
                        @foreach($tipos as $tipo)
                            <option value="{{ $tipo->id_tipo_ambulancia }}"
                                {{ old('id_tipo_ambulancia') == $tipo->id_tipo_ambulancia ? 'selected' : '' }}>
                                {{ $tipo->nombre_tipo }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="id_operador"
                        ...
                        @foreach($operadores as $operador)
                            <option value="{{ $operador->id_usuario }}"
                                {{ old('id_operador') == $operador->id_usuario ? 'selected' : '' }}>
